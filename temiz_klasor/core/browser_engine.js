...
async function navigateWithRetry(page, url, logger = console.log, maxRetries = 2, timeoutMs = 8000) {
  const delays = [1000, 2000, 3500];
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (hasContent) {
        ...
        if (!isCf) {
          return true; // Sayfa içeriği hazır!
        }
      }
      ...
      await page.waitForSelector('.predictioncontain, .schema, .rcnt, #m1x2_table, .mrows', { timeout: 1500 }).catch(() => null);
