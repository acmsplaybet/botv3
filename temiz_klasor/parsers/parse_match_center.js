...
async function parseMatchCenter(page, hero = {}) {
  ...
  // 0. Skip match center for upcoming matches that have no scores/events
  const isFinishedOrLive = hero && (
    (hero.result && (hero.result.status === 'FT' || hero.result.status === 'FINISHED' || hero.result.isLive)) ||
    (hero.finalScore && hero.finalScore !== '-' && !hero.finalScore.includes('?')) ||
    (hero.score && hero.score !== '-' && !hero.score.includes('?'))
  );

  if (!isFinishedOrLive && hero.homeTeam) {
    return matchCenter;
  }
  ...
  try {
    ...
    // Wait specifically for event rows or scoreboard (fast 600ms timeout for AJAX)
    const eventsAppeared = await page.waitForSelector('.ft-events, .match-events', { timeout: 600 }).then(() => true).catch(() => false);
    if (!eventsAppeared) {
      return matchCenter;
    }
