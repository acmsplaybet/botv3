...
async function parseHero(page) {
  return await page.evaluate(() => {
    ...
    // Extract Form Badges (Exactly 6 Forebet W/D/L Matches)
    const extractPrForm = (cont) => {
      if (!cont) return [];
      const spans = Array.from(cont.querySelectorAll('span'));
      return spans.map(s => {
        const txt = clean(s.querySelector('a')?.innerText || s.innerText);
        return txt;
      }).filter(t => ['W', 'D', 'L'].includes(t)).slice(0, 6);
    };

    const lCont = document.querySelector('div.lLogo .prformcont, .schema .homeTeam .prformcont, .homeLogo .prformcont');
    const rCont = document.querySelector('div.rLogo .prformcont, .schema .awayTeam .prformcont, .awayLogo .prformcont');

    let homeForm = extractPrForm(lCont);
    let awayForm = extractPrForm(rCont);
    ...
    if (homeForm.length === 0 || awayForm.length === 0) {
      ...
      if (prFormContainers.length >= 1 && homeForm.length === 0) {
        homeForm = extractPrForm(prFormContainers[0]);
      }
      if (prFormContainers.length >= 2 && awayForm.length === 0) {
        awayForm = extractPrForm(prFormContainers[1]);
      }
    }
    ...
    // Venue / Stadium
    const venueEl = document.querySelector('.schema .st_venue, .st_stadium, .venue_name, .st_venue, .schema_h2h .st_stadium, .weather_main_pr span:not(.wnums), .weather_main_pr');
    let venueText = clean(venueEl?.innerText).replace(/\s*\d+°.*$/, '').trim();
    if (!venueText) {
      ...
      for (const p of allP) {
        ...
        if (m) {
          venueText = clean(m[1]).replace(/\s*\d+°.*$/, '').trim();
          break;
        }
      }
    }
