...
async function parseMatchCenter(page, hero = {}) {
  ...
  try {
    ...
    // Wait specifically for event rows or scoreboard (3s timeout for AJAX)
    const eventsAppeared = await page.waitForSelector('.ft-events, .match-events', { timeout: 3000 }).then(() => true).catch(() => false);
    if (!eventsAppeared) {
      return matchCenter;
    }
