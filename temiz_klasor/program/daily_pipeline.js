...
async function scrapeSingleMatch(page, matchMeta, workerId) {
  ...
  const deepStats = await page.evaluate(() => {
    ...
    // Sakatlar ve Cezalılar (Injuries & Suspensions)
    const injuries = { home: [], away: [] };
    const injuryHeaders = Array.from(document.querySelectorAll('.mptlt, h3, .mod_title, b, strong')).filter(h => {
      return (h.innerText || '').toLowerCase().includes('injured and suspended');
    });
    if (injuryHeaders.length > 0) {
      const parentMod = injuryHeaders[0].closest('.moduletable') || injuryHeaders[0].closest('td') || injuryHeaders[0].parentElement;
      if (parentMod) {
        const teamBlocks = Array.from(parentMod.querySelectorAll('.sdl_team'));
        teamBlocks.forEach((tb, idx) => {
          const players = [];
          tb.querySelectorAll('.sdl_player').forEach(p => {
            const rawName = (p.querySelector('.sdl_player-name')?.innerText || '').replace(/\s+/g, ' ').trim();
            const reason = (p.querySelector('.sdl_injury')?.innerText || '').replace(/\s+/g, ' ').trim();
            const until = (p.querySelector('.sdl_until')?.innerText || '').replace(/\s+/g, ' ').trim() || '-';
            let pos = '';
            const posMatch = rawName.match(/\(([A-Z]+)\)$/);
            if (posMatch) pos = posMatch[1];
            if (rawName) players.push({ name: rawName, reason, until, position: pos });
          });
          if (idx === 0) injuries.home = players;
          else if (idx === 1) injuries.away = players;
        });
      }
    }
    ...
    async function workerTask(worker) {
      ...
      while (queueIndex < matchQueue.length) {
        ...
        try {
          ...
          // 1. match_data.json ve 1:1 HTML Viewer oluştur
          try {
