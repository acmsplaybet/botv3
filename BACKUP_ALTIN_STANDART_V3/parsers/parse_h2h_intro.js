...
async function parseH2HAndIntro(page, heroOrHome = '', heroAway = '') {
  let expHome = '';
  let expAway = '';
  if (typeof heroOrHome === 'object' && heroOrHome !== null) {
    expHome = heroOrHome.homeTeam || '';
    expAway = heroOrHome.awayTeam || '';
  } else {
    expHome = String(heroOrHome || '');
    expAway = String(heroAway || '');
  }
  ...
  return await page.evaluate((expHomeVal, expAwayVal) => {
    const clean = s => s ? s.replace(/\s+/g, ' ').trim() : '';

    let expHome = expHomeVal;
    let expAway = expAwayVal;

    if (!expHome) {
      expHome = clean(document.querySelector('.schema .homeTeam, .schema .tname:first-of-type, .schema .st_homeTeam')?.innerText) || '';
    }
    if (!expAway) {
      expAway = clean(document.querySelector('.schema .awayTeam, .schema .tname:last-of-type, .schema .st_awayTeam')?.innerText) || '';
    }
