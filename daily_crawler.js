      ...
      matches = await page.evaluate(() => {
        ...
        rows.forEach(row => {
          ...
          if (url) {
            ...
            const primaryOddEl = row.querySelector('.bigOnly.prmod .lscrsp, .prmod .lscrsp, .prmod span, span.lscrsp, .lodd, .odd, .podd, .l_od');
            ...
            const haoddSpans = Array.from(row.querySelectorAll('.haodd span, .haodd div, .haodd_div span'))
              .map(s => parseFloat((s.innerText || '').trim()))
              .filter(n => !isNaN(n) && n > 1.0);
            const hasHaodd = haoddSpans.length > 0;

            const hasValidOdd = hasPrimaryOdd || hasHaodd;
            ...
            list.push({
              ...
              shortTag: shortTag,
              flagUrl: flagSrc,
              listingOdd: detectedOdd,
              hasValidOdd: hasValidOdd,
...
async function runParallelPipelineForDate(dateStr, targetUrl) {
  ...
  try {
    ...
    const withOdds = allListingMatches.filter(m => m.hasValidOdd);
    const withoutOdds = allListingMatches.filter(m => !m.hasValidOdd);

    log(`📋 [BÜLTEN TARAMASI] Toplam ${totalFound} maç bulundu.`, COLORS.cyan);
    log(`🎯 [ORAN FİLTRESİ] ${withOdds.length} oranlı maç işleme alınıyor (${withoutOdds.length} oransız maç elendi).`, COLORS.green);

    let matchQueue = onlyWithOdds ? withOdds : allListingMatches;
      ...
      while (queueIndex < matchQueue.length) {
        ...
        try {
          ...
          try {
            ...
            const outBase = cfg.outputDir && fs.existsSync(cfg.outputDir) ? cfg.outputDir : path.join(__dirname, 'output');
            const rawSlug = matchData.match_slug || `match-${matchData.match_id}`;
            const cleanSlug = decodeURIComponent(rawSlug).trim();
            const matchDir = path.join(outBase, cleanSlug);
            if (!fs.existsSync(matchDir)) fs.mkdirSync(matchDir, { recursive: true });

            const stdData = {
              meta: {
                scrapedAt: matchData.scrape_meta?.scraped_at || new Date().toISOString(),
                durationSeconds: matchData.scrape_meta?.duration_sec || 3.5,
                sourceUrl: matchData.match_url || '',
                slug: cleanSlug
              },
              hero: {
                homeTeam: matchData.home_team?.name || 'Home',
                awayTeam: matchData.away_team?.name || 'Away',
                homeLogo: matchData.home_team?.logo_url || '',
                awayLogo: matchData.away_team?.logo_url || '',
                score: matchData.match_outcome?.final_score || '-',
                finalScore: matchData.match_outcome?.final_score || '-',
                league: matchData.league?.name || '',
                leagueFlag: matchData.league?.flag_url || '',
                matchDate: matchData.match_date || '',
                result: {
                  status: matchData.match_outcome?.status || 'Upcoming',
                  homeScore: matchData.match_outcome?.home_goals,
                  awayScore: matchData.match_outcome?.away_goals
                }
              },
              markets: matchData.predictions_9_tabs || {},
              h2h: matchData.h2h_summary || {},
              standings: matchData.standings || {},
              injuries: matchData.injuries || {},
              lastMatches: matchData.last_matches || {},
              overallStats: matchData.overall_stats || {},
              nextMatches: matchData.next_matches || {},
              matchCenter: matchData.match_center || {},
              raw_pipeline_data: matchData
            };

            const jsonPath = path.join(matchDir, 'match_data.json');
            const viewerPath = path.join(matchDir, 'viewer.html');
            fs.writeFileSync(jsonPath, JSON.stringify(stdData, null, 2), 'utf-8');
            generateMatchViewer(stdData, viewerPath);
          } catch (e) {
            console.error('Match output save error:', e.message);
          }
